...
architecture tb of riscv32_async_exception_handler_tb is
    ...
    signal global_interrupts_enabled : boolean := false;
    ...
    signal supervisor_level_external_interrupt_pending : boolean := false;
    signal supervisor_level_external_interrupt_enabled : boolean := false;

    signal supervisor_level_timer_interrupt_pending : boolean := false;
    signal supervisor_level_timer_interrupt_enabled : boolean := false;

    signal supervisor_level_software_interrupt_pending : boolean := false;
    signal supervisor_level_software_interrupt_enabled : boolean := false;
begin
    ...
    begin
        ...
        while test_suite loop
            if run("If machine level external interrupt pending, then the right async exception is pending") then
                ...
                supervisor_level_external_interrupt_enabled <= true;
                supervisor_level_external_interrupt_pending <= true;
                supervisor_level_software_interrupt_enabled <= true;
                supervisor_level_software_interrupt_pending <= true;
                supervisor_level_timer_interrupt_enabled <= true;
                supervisor_level_timer_interrupt_pending <= true;
                wait until falling_edge(clk);
                check(not async_exception_pending);
                ...
                wait until falling_edge(clk);
                check(async_exception_pending);
                check(async_exception_code = riscv32_exception_code_supervisor_external_interrupt);
                supervisor_level_external_interrupt_enabled <= false;
                wait until falling_edge(clk);
                check(async_exception_pending);
                check(async_exception_code = riscv32_exception_code_supervisor_software_interrupt);
                supervisor_level_software_interrupt_enabled <= false;
                wait until falling_edge(clk);
                check(async_exception_pending);
                check(async_exception_code = riscv32_exception_code_supervisor_timer_interrupt);
                supervisor_level_timer_interrupt_enabled <= false;
                wait until falling_edge(clk);
                check(not async_exception_pending);
                ...
                check(async_exception_pending);
                check(async_exception_code = riscv32_exception_code_supervisor_external_interrupt);
                supervisor_level_external_interrupt_pending <= false;
                wait until falling_edge(clk);
                check(async_exception_pending);
                check(async_exception_code = riscv32_exception_code_supervisor_software_interrupt);
                supervisor_level_software_interrupt_pending <= false;
                wait until falling_edge(clk);
                check(async_exception_pending);
                check(async_exception_code = riscv32_exception_code_supervisor_timer_interrupt);
                supervisor_level_timer_interrupt_pending <= false;
                wait until falling_edge(clk);
                check(not async_exception_pending);
            end if;
        end loop;
    ...
    port map (
        ...
        global_interrupts_enabled => global_interrupts_enabled,
        ...
        supervisor_level_external_interrupt_pending => supervisor_level_external_interrupt_pending,
        supervisor_level_external_interrupt_enabled => supervisor_level_external_interrupt_enabled,
        supervisor_level_timer_interrupt_pending => supervisor_level_timer_interrupt_pending,
        supervisor_level_timer_interrupt_enabled => supervisor_level_timer_interrupt_enabled,
        supervisor_level_software_interrupt_pending => supervisor_level_software_interrupt_pending,
        supervisor_level_software_interrupt_enabled => supervisor_level_software_interrupt_enabled
    );
end architecture;
