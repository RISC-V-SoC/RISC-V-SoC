...
architecture behavioral of platform_level_interrupt_controller is
    ...
    type is_interrupt_claimed_vector_type is array(1 to interrupt_source_count - 1) of boolean;
    signal is_interrupt_claimed_vector : is_interrupt_claimed_vector_type := (others => false);
            ...
            if reset then
                ...
                if base_address mod 4 /= 0 then
                    ...
                    if base_address >= 16#001000# and base_address < 16#001080# then
                        ...
                        slv2mst_tmp.readData := bool_vec_to_std_logic_word(interrupt_pending_vector, address * 8);
                    end if;
                    ...
                    if base_address >= 16#200000# and base_address <= 16#3FFF000# then
                        ...
                        if context_id < context_count and address mod 16#1000# = 4 then
                            ...
                            if is_write_op and interrupt_id > 0 and interrupt_id < interrupt_source_count then
